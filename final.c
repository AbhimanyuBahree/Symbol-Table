...
//if we do not know the scope it is global
//if we do not know noa it is -1
//rest all is NULL
//argument is <id,hash(id)>
//keywords have size,type and name
//identifiers have name,type,size,scope
//function identifiers have everything
//operators have name,size,type
//string literals "" ,'' have name,type,size
//
...
{
printf("%s\t%s\t%d\t%c\t%d\t%s\t%s\t\n",sym.name,sym.type,sym.size,sym.scope,sym.noa,sym.arguments,sym.returntype);
}
void Initialize()
{
...
{ int i=0;
          ...
          while(i<lindex)
          {
            ...
            if(lexemes[i][0]=='('){acount++;}

            if(lexemes[i][0]==')'){acount2++;}
            ...
            if(validIdentifier(lexemes[i]) && dou%2==0 && a%2==0)
            {
              if(i<=lindex-2 && lexemes[i+1][0]=='(')
               {printf("%s Function i= %d\n",lexemes[i],i);
                     ...
                     char ch='G';if(globalcount != globalcount2 || acount!=acount2)  ch='L';
                     char * arguments;
                     if(noa == 0)
                      {arguments=NULL;}
                    else{
                    ...
                    char * f="<id,";
                       arguments=f;
                       int a=Hash(lexemes[i]);
                         char * g=integer_to_string(a);
                            char * t=concatstr(f,g);
                            arguments=concatstr(t,">");
                       }
                     //char * g=concatchar(e,f);
                     //char * j=concatstr(g,">");
                   Insert(CreateSymbol(lexemes[i],"FUNC",sizer(type),'G',noa,arguments,type));
          }
...
int main()
{
char * filename1="la2.c";
char delimiter='@';
char * filename=preprocess(filename1); //q4out3.c
